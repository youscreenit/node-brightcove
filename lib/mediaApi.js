...
var api = function MediaApi(token, opts) {
	...
	this.ReadService = 'https://api.brightcove.com/services/library';
	this.WriteService = 'https://api.brightcove.com/services/post';
...
api.prototype.getUploadStatus = function getUploadStatus(options, callback) {

    this.makeRequest(this.commands.write.get_upload_status, options, callback);
}

api.prototype.createVideo = function createVideo(videoJson, fileName, fileReadStream, callback, progressCb) {
	var command = this.commands.write.create_video;

	if (typeof callback === undefined || typeof callback === null)
		throw new Error('no callback defined when calling makeRequest');

	var params = { params: { video: videoJson, filename: fileName, token: this.getToken() }, method: command };
    ...
    if ((typeof(progressCb) === 'function')) {
        fileReadStream.on('data', progressCb);
    }
